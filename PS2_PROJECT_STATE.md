...
- [ ] ALPHA `LoadDirectory` currently fails to resolve `DIR.BIN` at runtime and reports a stale prototype folder path ending in `(Mar 6, 2002 prototype)`. Until that media-path normalization is fixed, the new `OpenCDFile` / `ReadCDSectorASync` trace wrappers cannot reach the original `sceCdRead` blocker.
